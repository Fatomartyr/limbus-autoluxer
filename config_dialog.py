...
class FarmConfigDialog:
    """Configuration dialog for farm settings"""
    ...
    def get_farm_config(self) -> Optional[dict]:
        ...
        click_count_var = tk.IntVar(value=2)

        tk.Radiobutton(
            click_frame,
            text="EXP",
            variable=click_count_var,
            value=2,
            font=("Arial", 10)
        ...
        if self.result and self.stage_selector:
            click_count = self.result.get("click_count", 2)
            positions = self._show_stage_selection_dialog(click_count)
            if positions:
                self.result["stage_positions"] = positions

        return self.result

    def _show_stage_selection_dialog(self, click_count: int = 2) -> Optional[List[Tuple[int, int]]]:
        ...
        for i in range(1, click_count + 1):
            ...
            pos = self.stage_selector.wait_for_user_click(
                timeout=1800,
                click_number=i,
            ...
            if pos is None:
                messagebox.showerror("Ошибка", f"Не удалось получить клик #{i}")
                wait_dialog.destroy()
                return None
